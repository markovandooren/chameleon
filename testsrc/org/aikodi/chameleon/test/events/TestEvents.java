...
public class TestEvents {
	...
	protected NamespaceDeclaration namespaceDeclaration() {
		return _document.lexical().children(NamespaceDeclaration.class).get(0);
	}
	...
	public void createProject() throws ProjectException {
		...
		projectBuilder.withLanguage(_language, v -> {
			_view = v;
		}).project();
		...
		}).builder().declaration("a", f).container("b", f).declaration("a", f).declaration("b", f).done().done()
				.document();
	}
	...
	public void testRemoveAndRename() {
		...
		documentAnyChecker.expect(Removed.class, namespaceDeclaration());
		// The signature will not be disconnected, so we expect only the parent removed
		// event.
		aChecker.expect(ParentRemoved.class, a);
	...
	public void testReplace() {
		...
		aChecker.expect(ParentAdded.class, a);
		declaration.parentLink().getOtherRelation().replace((Association) declaration.parentLink(),
				(Association) a.parentLink());
		aChecker.check();
